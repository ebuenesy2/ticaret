...
<?php // echo "<pre>"; print_r($DB_Find_Stock); die(); ?>
                ...
                <form action="#">
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="col-lg-6 mb-3"> 
                                <label for="selectStockAdd" class="form-label">Firma Seçiniz</label>
                                <select class="form-select"  id="selectStockAdd">
                                    <option value="">Stok Kart Seç</option>
                                    @for ($i = 0; $i < count($DB_Find_Stock); $i++) <option value="{{$DB_Find_Stock[$i]->id}}"  >{{$DB_Find_Stock[$i]->nameTr}}</option>  @endfor
                                </select>
                            </div>
                            <!-- Stok Bilgileri Son --> 
                    ...
                    <div class="modal-footer">
                        <div class="hstack gap-2 justify-content-end">
                            ...
                            <button type="button" class="btn btn-success" id="new_add_stock" data_current_id="{{$CurrentId}}" >@lang('admin.Add')</button>
                        </div>
                    </div>
                </form>
            </div>
                ...
                <form action="#">
                    ...
                    <div class="modal-body" id="ModalBodyUpdate" style="display:none;" >
                        <div class="row">
                            ...
                            <!-- Stok Bilgileri --> 
                            <div class="col-lg-6 mb-3"> 
                                <label for="selectCurrentStockEdit" class="form-label">Firma Seçiniz</label>
                                <select class="form-select"  id="selectCurrentStockEdit">
                                    <option value="">Stok Kart Seç</option>
                                    @for ($i = 0; $i < count($DB_Find_Stock); $i++) <option value="{{$DB_Find_Stock[$i]->id}}"  >{{$DB_Find_Stock[$i]->nameTr}}</option>  @endfor
                                </select>
                            </div>
                    ...
                    <div class="modal-footer">
                        <div class="hstack gap-2 justify-content-end">
                            ...
                            <button type="button" class="btn btn-info" id="data_update_stock">@lang('admin.Update')</button>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <div class="row">
            ...
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    <h4 class="mb-sm-0">Firma #{{$CurrentId}} -  [ {{$DB_Find_Current->current_name}} ] </h4>

                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            <li class="breadcrumb-item"><a href="/stock/@lang('admin.lang')">@lang('admin.Stock')</a></li>
                            <li class="breadcrumb-item active">Firma Stokları</li>
                        </ol>
                    </div>
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header d-flex align-items-center border-0">
                                <h5 class="card-title mb-0 flex-grow-1" style="display: flex;gap: 5px;" > <p id="tableTitle" >Firma Stok Listesi</p> <p> | {{count($DB_Find)}}</p> 
                                    ...
                                    <table class="table align-middle table-nowrap" id="customerTable">
                                        <thead class="table-light text-muted">
                                            <tr style="cursor:pointer;" >
                                                ...
                                                <th exportname="current_cartId" class="sort" data-sort="order_date" scope="col" >Stok Id</th>
                                                <th exportname="current_cartCode" class="sort" data-sort="order_date" scope="col" >Stok Kod</th>
                                                <th exportname="current_cartName" class="sort" data-sort="order_date" scope="col" >Stok Kart Adı</th>
                                        ...
                                        <tbody class="list form-check-all">
                                            ...
                                            @for ($i = 0; $i < count($DB_Find); $i++)
                                                <tr>
                                                    ...
                                                    <td exportname="current_cartId" > {{$DB_Find[$i]->stock_id}}</td>
                                                    <td exportname="current_cartCode" > {{$DB_Find[$i]->stockCode}}</td>
                                                    <td exportname="current_cartName" > {{$DB_Find[$i]->stockNameTR}}</td>
                                                    <td exportname="CurrentRow" > {{$DB_Find[$i]->current_row == 120 ? 'Alıcı' : 'Satıcı'}}</td>
                                                 

                                                    <td exportname="Actions" id="listItemActionBox" > 
                                                        <ul class="list-inline hstack gap-2 mb-0">
                                                            <li class="list-inline-item edit" title ="@lang('admin.Update')" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" aria-label="Edit"  style="cursor:pointer;"> <a data-bs-toggle="modal" data-bs-target="#updateModal" data-id="{{$DB_Find[$i]->id}}" data-created_at="{{$DB_Find[$i]->created_at}}" data-isActive="{{$DB_Find[$i]->isActive}}"  class="text-primary d-inline-block edit-item-btn"> <button class="btn btn-primary waves-effect waves-light" style="width: 45px;height: 45px;"><i class="ri-pencil-fill fs-16"></i> </button> </a> </li>
