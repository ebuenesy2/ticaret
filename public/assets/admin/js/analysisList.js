...
var paginationNext = (ContactList && (contactList = new List("contactList", options).on("updated", function (e) { 0 == e.matchingItems.length ? document.getElementsByClassName("noresult")[0].style.display = "block" : document.getElementsByClassName("noresult")[0].style.display = "none"; var t = 1 == e.i, n = e.i > e.matchingItems.length - e.page; document.querySelector(".pagination-prev.disabled") && document.querySelector(".pagination-prev.disabled").classList.remove("disabled"), document.querySelector(".pagination-next.disabled") && document.querySelector(".pagination-next.disabled").classList.remove("disabled"), t && document.querySelector(".pagination-prev").classList.add("disabled"), n && document.querySelector(".pagination-next").classList.add("disabled"), e.matchingItems.length <= perPage ? document.querySelector(".pagination-wrap").style.display = "none" : document.querySelector(".pagination-wrap").style.display = "flex", 0 < e.matchingItems.length ? document.getElementsByClassName("noresult")[0].style.display = "none" : document.getElementsByClassName("noresult")[0].style.display = "block" }), isCount = (new DOMParser).parseFromString(contactList.items.slice(-1)[0]._values.id, "text/html")), document.querySelector(".pagination-next")), paginationPrev = (paginationNext && document.querySelector(".pagination-next").addEventListener("click", function () { !document.querySelector(".pagination.listjs-pagination") || document.querySelector(".pagination.listjs-pagination").querySelector(".active") && document.querySelector(".pagination.listjs-pagination").querySelector(".active").nextElementSibling.children[0].click() }), document.querySelector(".pagination-prev")); paginationPrev && document.querySelector(".pagination-prev").addEventListener("click", function () { !document.querySelector(".pagination.listjs-pagination") || document.querySelector(".pagination.listjs-pagination").querySelector(".active") && document.querySelector(".pagination.listjs-pagination").querySelector(".active").previousSibling.children[0].click() });
        ...
        //! Delete
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    //! Ajax
                    $.ajax({
                        url: "/analysis/list/delete/post/multi",
                        method: "post",
    ...
    //! Arama Ürün Seçme
    document.querySelector('#selectProduct').addEventListener('change', e => {

        //! Tanım
        var filter = $('#selectProduct').val(); //! Aranacak Veri
        var searchJsonItem = { exportname: "Product", text:filter}; //! Aranacak Veri Item
        var searchJsonFindItem = searchJsonData.findIndex(s => s.exportname == 'Product'); //! Json İçinde Arama 
    ...
    function JsonHtml_Controller () {
        ...
        searchJsonData.map(function(data){
            ...
            else if(data.exportname == "Product") {  $('#selectProduct option[value="'+data.text+'"]').prop('selected', true );  } //! Seçim yap
    ...
    $("#new_add").click(function (e) {
        ...
        //! Bilgiler
        var productItemsAdd =  $('#productItemsAdd').val();
        var platformAdd =  $('#platformAdd').val();
        ...
        if(productItemsAdd == "") { 
            ...
            Swal.fire({
                position: "center",
                icon: "error",
                title: 'Araştırma Platform Seçiniz',
                showConfirmButton: false,
                timer: 2000,
            });
        } 
            ...
            //! Ajax
            $.ajax({
                url: "/analysis/list/add/post",
                method: "post",
                headers: { "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"), },
                data: {
                    siteLang: $('[id=lang_change][data_key=lang]').html().trim(),
                    product_id: $('#productItemsAdd').val(),
                    pratform_id: $('#platformAdd').val(),
        ...
        $("#edit_modal").modal({
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    //! Piyasa Analizi
                    $('#productPriceEdit').val(response.DB.productPrice);

                    $('#systemCommissionEdit').val(response.DB.systemCommission);
                    $('#systemCommissionTotalEdit').val(response.DB.systemCommissionTotal);
                    $('#systemKdvEdit').val(response.DB.vat);
                    //$('#vatTotalEdit').val(response.DB.vatTotal);

                    $('#profitTaxEdit').val(response.DB.profitTax);
                    $('#profitTaxTotalEdit').val(response.DB.profitTaxTotal);
                    $('#shippingCostEdit').val(response.DB.shippingCost);
                    $('#shippingCostTotalEdit').val(response.DB.shippingCostTotal);

                    $('#inventoryCostEdit').val(response.DB.inventoryCost);
                    $('#inventoryCostTotalEdit').val(response.DB.inventoryCostTotal);
                    $('#salesProfitRateEdit').val(response.DB.salesProfitRate);
                    $('#salesProfitRateTotalEdit').val(response.DB.salesProfitRateTotal);
                    $('#netPriceEdit').val(response.DB.netPrice);
    ...
    $("#edit_item").click(function (e) {
        ...
        //! Bilgiler
        var productItemsEdit =  $('#productItemsEdit').val();
        var platformEdit =  $('#platformEdit').val();
        ...
        if(productItemsEdit == "") { 
            ...
            $.ajax({
                ...
                data: {
                    ...
                    product_id: $('#productItemsEdit').val(),
                    pratform_id: $('#platformEdit').val(),
                    ...
                    systemCommission : $('#systemCommissionEdit').val(),
                    systemCommissionTotal: $('#systemCommissionTotalEdit').val(),
                    vat: $('#systemKdvEdit').val(),
                    vatTotal: $('#vatTotalEdit').val(),
 
                    profitTax: $('#profitTaxEdit').val(),
                    profitTaxTotal: $('#profitTaxTotalEdit').val(),
                    shippingCost: $('#shippingCostEdit').val(),
                    shippingCostTotal: $('#shippingCostTotalEdit').val(),
 
                    inventoryCost: $('#inventoryCostEdit').val(),
                    inventoryCostTotal: $('#inventoryCostTotalEdit').val(),
                    salesProfitRate: $('#salesProfitRateEdit').val(),
                    salesProfitRateTotal: $('#salesProfitRateTotalEdit').val(),
                    netPrice: $('#salesProfitRateTotalEdit').val(),
    ...
    //! Hesaplamalar
    document.querySelectorAll("#PriceEdit").forEach(function (i) { i.addEventListener("keyup", function (event) { general(); }); }); //! Ürün Fiyatı

    document.querySelectorAll("#systemKdvEdit").forEach(function (i) { i.addEventListener("keyup", function (event) { general(); }); }); //! Sistem Komisyonu
    document.querySelectorAll("#systemCommissionEdit").forEach(function (i) { i.addEventListener("keyup", function (event) { general(); }); }); //! Sistem Komisyonu
    document.querySelectorAll("#vatEdit").forEach(function (i) { i.addEventListener("keyup", function (event) { general(); }); }); //! Sistem Komisyonu
    document.querySelectorAll("#profitTaxEdit").forEach(function (i) { i.addEventListener("keyup", function (event) { general(); }); }); //! Sistem Komisyonu
    document.querySelectorAll("#shippingCostEdit").forEach(function (i) { i.addEventListener("keyup", function (event) { general(); }); }); //! Sistem Komisyonu
    document.querySelectorAll("#inventoryCostEdit").forEach(function (i) { i.addEventListener("keyup", function (event) { general(); }); }); //! Sistem Komisyonu
    document.querySelectorAll("#salesProfitRateEdit").forEach(function (i) { i.addEventListener("keyup", function (event) { general(); }); }); //! Sistem Komisyonu


    function general(){
       
        //! Ürün Fiyatı
        var Price = $('#PriceEdit').val().replace(',','.'); //! Sayıya Çevirme
       
        //! Sistem Kdv
        var systemKdv = $('#systemKdvEdit').val().replace(',','.'); //! Sayıya Çevirme
        var systemKdv_tutar = Number(Price)/((100+Number(systemKdv))/100); //! Hesaplama
        var systemCommission_kalan = Number(Price) - Number(systemKdv_tutar); //! Kalan
        $('#systemKdvTotalEdit').val(systemCommission_kalan.toFixed(2)); //! Veri Gösteriyor
        $('#systemKdvOtherEdit').val(systemKdv_tutar.toFixed(2)); //! Veri Gösteriyor

        //! Sistem Komisyonı
        var systemCommission = $('#systemCommissionEdit').val().replace(',','.'); //! Sayıya Çevirme
        var systemCommission_tutar = (Number(Price)*Number(systemCommission))/100; //! Hesaplama
        var systemCommission_kalan = Number(systemKdv_tutar) - Number(systemCommission_tutar); //! Kalan
        $('#systemCommissionTotalEdit').val(systemCommission_tutar.toFixed(2)); //! Veri Gösteriyor

        //! Kargo Maliyeti
        var shippingCost = $('#shippingCostEdit').val().replace(',','.'); //! Sayıya Çevirme
        var shippingCost_tutar = (Number(Price)*Number(shippingCost))/100; //! Hesaplama
        var shippingCost_kalan = Number(systemCommission_kalan) - Number(shippingCost_tutar); //! Kalan
        $('#shippingCostTotalEdit').val(shippingCost_tutar.toFixed(2)); //! Veri Gösteriyor

        //! Satıcının Eline Geçen
        $('#sellerNetPrice').val(shippingCost_kalan.toFixed(2)); //! Brüt Tutar

        //! Satış Kar Oranı
        var salesProfitRate = $('#salesProfitRateEdit').val().replace(',','.'); //! Sayıya Çevirme
        var salesProfitRate_tutar = Number(shippingCost_kalan)/((100+Number(salesProfitRate))/100); //! Hesaplama
        var salesProfitRate_kalan = Number(shippingCost_kalan) - Number(salesProfitRate_tutar); //! Kalan
        $('#salesProfitRateTotalEdit').val(salesProfitRate_tutar.toFixed(2)); //! Veri Gösteriyor

        //! Brüt Kar
        $('#productBrutVatEdit').val(salesProfitRate_kalan.toFixed(2)); 

        
        //! Depo Maliyeti
        var inventoryCost = $('#inventoryCostEdit').val().replace(',','.'); //! Sayıya Çevirme
        var inventoryCost_tutar = (Number(salesProfitRate_tutar)*Number(inventoryCost))/100; //! Hesaplama
        $('#inventoryCostTotalEdit').val(inventoryCost_tutar.toFixed(2)); //! Veri Gösteriyor

       
        //! KV ve KDV 
        var profitTax = $('#profitTaxEdit').val().replace(',','.'); //! Sayıya Çevirme
        var profitTax_tutar = (Number(salesProfitRate_kalan)*Number(profitTax))/100; //! Hesaplama
        var profitTax_tutar = profitTax_tutar+profitTax_tutar; //! Hesaplama
        $('#profitTaxTotalEdit').val(profitTax_tutar.toFixed(2)); //! Veri Gösteriyor

        //! Diğer Maliyetlerin Toplamı
        var otherCostTotar = Number(profitTax_tutar) + Number(inventoryCost_tutar);
        $('#otherCostTotarEdit').val(otherCostTotar.toFixed(2)); //! Veri Gösteriyor

        //! Net Kar
        var netProfit = Number(salesProfitRate_kalan) -Number(otherCostTotar); //! Net Kar
        var netProfitPer = Number(Price) / Number(netProfit); //! Net Kar Yüzdesi
        $('#netProfitEdit').val(netProfit.toFixed(2)); //! Veri Gösteriyor
        $('#netProfitPercEdit').val(netProfitPer.toFixed(2)); //! Veri Gösteriyor

        // console.log("Price:",Price);
        // console.log("netProfit:",netProfit);

        //! Yüzdelik Kar
        var profitPercen = (100*netProfit)/Price;
        $('#netProfitPercEdit').val(profitPercen.toFixed(2)); //! Veri Gösteriyor
