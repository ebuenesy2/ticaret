...
$('input[type="checkbox"][id="showAllRows"]').click(function () {
    ...
    $("input[type=checkbox][id=checkItem]").prop('checked', $(this).prop('checked'));
    
    //! Elemanları alıyor
    var eleman = $("input[type=checkbox][id=checkItem]");
    ...
    for (var i = 0; i < eleman_sayisi; i++) {
        var eleman_id = eleman[i].attributes['data_check_id'].value
        var ischecked = eleman[i].checked;

        if(ischecked) { checkItemsArray.push(eleman_id); }
    }
...
$('input[type="checkbox"][id="checkItem"]').click(function () {
    ...
    //! Elemanları alıyor
    var eleman = $("input[type=checkbox][id=checkItem]"); //! Veri Oku
    var eleman_sayisi = eleman.length; //! Eleman Sayısı
    var SelectAllStatus = true; //! Tüm Durum

    var allCheckItems = $('#showAllRows').attr('data_value'); //! 0,1,2
    var allCheckItemsArray = allCheckItems.split(',');
    ...
    for (var i = 0; i < eleman_sayisi; i++) {
        ...
        //! Seçili Değilse
        if(ischecked==false) { 
            var SelectAllStatus = false; //! Tümü Seçme Kaldırma
            allCheckItemsArray.includes(eleman_id) && allCheckItemsArray.splice(allCheckItemsArray.indexOf(eleman_id ),1); //! Silme
        } //! Seçili Değilse Son

        //! Seçili
        else if(ischecked==true) {  
            !allCheckItemsArray.includes(eleman_id) && allCheckItemsArray.push(eleman_id); //! Ekleme İşlemi
        } //! Seçili  Son
...
function choosePanelView() {
    ...
    $('#choosedItemCount').html(allCheckItemsArrayLength);
...
//! ************ Tümü Seç Olayı Son ***************
    ...
    function JsonHtml_Controller () {
        ...
        searchJsonData.map(function(data){
            ...
            else if(data.exportname == "Departman") {  $('#selectDepartmanSearch option[value="'+data.text+'"]').prop('selected', true );  } //! Seçim yap
            else if(data.exportname == "Role") {  $('#selectRoleSearch option[value="'+data.text+'"]').prop('selected', true );  } //! Seçim yap
   ...
   //! ************ Arama  ***************
    ...
    function searchTableControl(){
        ...
        //! Sil
        console.log("searchParamsList:",searchParamsList);
    ...
    //! Arama Departman
    document.querySelector('#selectDepartmanSearch').addEventListener('change', e => {

        //! Tanım
        var filter = $('#selectDepartmanSearch').val(); //! Aranacak Veri
        var searchJsonItem = { exportname: "Departman", text:filter}; //! Aranacak Veri Item
        var searchJsonFindItem = searchJsonData.findIndex(s => s.exportname == 'Departman'); //! Json İçinde Arama 
    ...
    //! Arama Role
    document.querySelector('#selectRoleSearch').addEventListener('change', e => {

        //! Tanım
        var filter = $('#selectRoleSearch').val(); //! Aranacak Veri
...
$('document').ready(function () {
    $("#edit_modal").modal({
        ...
        var modal_Surname = button.data("surname"); 

        var modal_Email = button.data("email"); 
        var modal_Tel = button.data("tel"); 
        var modal_Password = button.data("password"); 


        var modal_departmanId = button.data("departmanid"); 
        var modal_role = button.data("role"); 

        console.log("modal_departman:",modal_departmanId);
        console.log("modal_role:",modal_role);
        ...
        $('#UpdatedEmail').val(modal_Email);
        $('#UpdatedTel').val(modal_Tel);
        $('#UpdatedPassword').val(modal_Password);

        $('#selectDepartmanUpdate option[value='+modal_departmanId+']').prop('selected',true);
        $('#selectRoleUpdate option[value='+modal_role+']').prop('selected',true);
...
$("#edit_item").click(function (e) {
    ...
    var selectRole = $('#selectRoleUpdate').val();
    ...
    if(selectDepartman == "") {
        ...
        $.ajax({
            ...
            data: {
                ...
                surname: $('#UpdatedSurname').val(),
                email: $('#UpdatedEmail').val(),
                tel: $('#UpdatedTel').val(),
                password: $('#UpdatedPassword').val(),
                repassword: $('#UpdatedRePassword').val(),

                departmanId: $('#selectDepartmanUpdate').val(),
                departman: $('#selectDepartmanUpdate option[value='+selectDepartman +']').html(),
                role: $('#selectRoleUpdate').val(),
