$(function () {
    ...
    //! ************ Ürün Bilgileri Modal ***************
        ...
        $("#product_add_item").click(function (e) {
            ...
            if(nameTrAdd == "") { 
                ...
                //! Ajax
                $.ajax({
                    url: "/get/offers/product/add/post",
                    method: "post",
                    headers: { "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"), },
                    data: {
                        siteLang: $('[id=lang_change][data_key=lang]').html().trim(),
                        get_offers_id: Number(data_id_req),
                        ...
                        requestform_product_id: Number($('#stockAdd').val()),
                    ...
                    success: function (response) {
                        // alert("başarılı");
                        //console.log("response Ekle:", response);
                        // console.log("status:", response.status);
            ...
            $("#Delete_ProductModal").modal({
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        //! Ajax
                        $.ajax({
                            url: "/get/offers/product/delete/post",
                            method: "post",
                            headers: { "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"), },
                            data: {
                                siteLang: $('[id=lang_change][data_key=lang]').html().trim(),
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if(response.status == "success") {
                            ...
                            $('#stockEdit option[value='+response.DB.requestform_product_id+']').prop('selected',true); //! Select
                            $('#stockEdit').attr('data_sector',response.DB.sector);
                            $('#stockEdit').attr('data_sub_sector',response.DB.sub_sector);
                            $('#stockEdit').attr('data_stock_id',response.DB.stock_id);
    ...
    $("#edit_item").click(function (e) {
        ...
        var offerEffectiveDate = $('#offerEffectiveDate').val(); //! Teklif Geçerlilik Tarihi
        ...
        // }
            ...
            $.ajax({
                ...
                data: {
                    ...
                    productTotal: $('#productTotalPayment').val(),
    ...
    //! ************  Maliyet Kalemi  *********************
      //! Ekleme
      $("#new_calculation_add").click(function (e) {
        e.preventDefault();

        var title_Val = $('#cal_titleAdd').val(); //! Başlık
        var currency_Val = $('#cal_CurrencyAdd').val(); //! Para Birimi
        var currencyRate_Val = $('#cal_CurrencyRateAdd').val(); //! Döviz Kuru
        var offerEffectiveDate = $('#offerEffectiveDate').val(); //! Teklif Geçerlilik Tarihi
        ...
        // }
            ...
            $.ajax({
                ...
                data: {
                    siteLang: $('[id=lang_change][data_key=lang]').html().trim(),
                    title: $('#cal_titleAdd').val(),
                    get_offers_id:Number(data_id),
                    currency: $('#cal_CurrencyAdd').val(),
                    currency_rate: $('#cal_CurrencyRateAdd').val(),
